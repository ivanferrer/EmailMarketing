    ...
    public function __construct() {
           ...
           $htaccess = "{$this->config['uploadDir']}/.htaccess";
            if (!file_exists($htaccess)) {
                if (!@file_put_contents($htaccess, $this->get_htaccess()))
                    $this->backMsg("Não foi possível localizar a pasta de upload para este caminho: {$this->config['uploadDir']}");
            } else {
                if (false === ($data = @file_get_contents($htaccess)))
                    $this->backMsg("Não foi possível ler o .htaccess");
                if (($data != $this->get_htaccess()) && !@file_put_contents($htaccess, $data))
                    $this->backMsg("Arquivo .htaccess está incorreto. Não é possível reescrever isto!");
            }
        }
        ...
        if (!is_dir($this->typeDir)) {
            if (!mkdir($this->typeDir, $this->config['dirPerms']))
                $this->backMsg("Não foi possível criar o diretório: {dir}", array('dir' => $this->type));
        } elseif (!is_readable($this->typeDir))
            $this->backMsg("Não é possível ler a pasta de upload.");
    }
